...
class SummaryController extends Controller
{
    ...
    public function documents(SummaryDocumentsRequest $request) {
        ...
        $documents = Document::query()
            //->leftJoin('voidedtikets', 'documents.id', '=', 'voidedtikets.document_id')
        ...
        $voidedtikets = Voidedtiket::all();

        foreach ($documents as $key => $document) {
            foreach ($voidedtikets as $voidedtiket) {
              if($document->id == $voidedtiket->document_id) {
                unset($documents[$key]);
              }
        }
