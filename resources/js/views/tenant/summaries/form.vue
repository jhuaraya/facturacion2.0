  ...
  <div>
                    ...
                    <tr v-for="(row, index) in form.documents">
                      ...
                      <td class="text-right">
                        <button
                          ...
                          @click.prevent="showAnnulmentReason(index,row.id)"
                        >x</button>
                      </td>
                    </tr>
                  </tbody>
    ...
    >
      <form @submit.prevent="submit">
        <div class="form-body">
          <div class="form-group"  :class="{'has-danger': errors.motivo}">
            <label for="exampleFormControlTextarea1">Ingrese aquí el motivo de anulación</label>
            <textarea v-model="reasonForm.motivo" class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
          </div>
        </div>
        <div class="form-actions text-right pt-2">
          <el-button @click.prevent="closeReasonDialog()">Cancelar</el-button>
          <el-button
            type="primary"
            native-type="submit"
            :loading="loading_submit"
            @click.prevent="saveReasonAnnulment"
            dusk="save-summary"
          >Guardar</el-button>
        </div>
      </form>
...
export default {
  ...
  methods: {
    ...
    saveReasonAnnulment(){
      this.$http
        .post(`/${this.resource}/annulmentballot`, {
          document_id: this.reasonForm.document_id,
          motivo: this.reasonForm.motivo,
        })
        .catch(error => {
          this.$message.error(error.response.data.message);
        })
        .then(() => {
          this.form.documents.splice(this.currenIndex, 1);
    ...
    showAnnulmentReason(index,idItem) {
      this.reasonForm.document_id = idItem;
